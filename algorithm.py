...
def save_result(HDR_result):
    ...
    cv.imwrite("./Test Images/3/Inconsistent/Result.jpg", result_image)
...
if __name__ == "__main__":
    time = np.array([1.0/2134.0, 1.0/201.0, 1.0/40.0, 1.0/5.0], dtype=np.float32)
    paths = ["./Test Images/3/Inconsistent/1.jpg",
             "./Test Images/3/Inconsistent/2.jpg",
             "./Test Images/3/Inconsistent/3.jpg",
             "./Test Images/3/Inconsistent/4.jpg"]
    images = readImagesAndTimes(paths)
    merge_mertens = cv.createMergeMertens()
    fusion = merge_mertens.process(images)
    fusion_8bit = np.clip(fusion * 255, 0, 255).astype('uint8')
    cv.imwrite("./Test Images/3/Inconsistent/fusion_result.png", fusion_8bit)
    ...
    plt.savefig('./Test Images/3/Inconsistent/CRF.png')
    plt.show()
